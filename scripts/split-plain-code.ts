...
async function main() {
  const { bundlePath, outputDirectory, isDryRun, shouldStripMdFences } =
    parseArguments(process.argv.slice(2));

  if (!bundlePath) {
    // Updated usage message to avoid deprecated tags
    console.info(
      'Usage: split-plain-code.ts <bundlePath> [--out <dir>] [--dry-run] [--strip-md-fences]',
    );
  ...
  if (sections.length === 0) {
    throw new Error(
      'No sections found. Make sure your file contains lines like: // File: relative/path',
    );
  }
  ...
  sections.forEach((section) => {
    const content = shouldStripMdFences
      ? maybeStripMdFence(section.content)
      : section.content;
  ...
  [...byRelativePath.entries()].forEach(([relativePath, content]) => {
    ...
    if (isDryRun) {
      ...
      console.info(
        `[dry-run] write ${path.relative(process.cwd(), abs)} (${size} bytes)`,
      );
    } else {
      writes.push(
        fs
          ...
          .then(() => {
            ...
            console.info(
              `write ${path.relative(process.cwd(), abs)} (${size} bytes)`,
            );
          }),
      );
    }
