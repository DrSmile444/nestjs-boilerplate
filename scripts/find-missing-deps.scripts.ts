...
const BUILTIN_SET = new Set([
  ...
  ...builtinModules
    ...
    .map((nodeModule) =>
      nodeModule.startsWith('node:') ? nodeModule.slice(5) : nodeModule,
    ),
]);
...
function detectPackageManager(
  ...
  forced?: CliOptions['packageManager'],
) {
  if (forced) {
    return forced;
  }

  const has = (filePath: string) =>
    safeExists(path.join(projectDirectory, filePath));
  ...
  try {
    ...
    const packageFile = JSON.parse(
      fs.readFile(packageJsonPath, 'utf8') as unknown as string,
    );
...
function isInstalledInNodeModules(
  projectDirectory: string,
  topLevel: string,
): boolean {
...
async function collectReferencedFromFile(
  file: string,
  tsconfigAliases: string[],
...
async function findUninstalledDeps(root: string): Promise<{
  ...
  const uninstalled = [...referenced].filter(
    (packageFile) => !isInstalledInNodeModules(root, packageFile),
  );
...
async function installDependencies(
  ...
  return new Promise<number>((resolve, reject) => {
    const child = spawn(cmd, parsedArguments, {
      stdio: 'inherit',
...
function parseArguments(argv: string[]): CliOptions {
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (option === '--dry-run') {
      ...
      out.packageManager = option.slice(
        '--package-manager='.length,
      ) as CliOptions['packageManager'];
...
async function main() {
  ...
  const { uninstalled, referencedTopLevel } = await findUninstalledDeps(
    options.dir,
  );
  ...
  console.info(
    `📚 Referenced packages (top-level): ${referencedTopLevel.size}`,
  );
