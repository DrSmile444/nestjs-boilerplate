...
async function main() {
  ...
  if (!hasSource) {
    throw new Error(
      `Cannot find "${SRC_DIR_NAME}" folder at: ${sourceDirectoryAbs}`,
    );
  }
  ...
  if (sourceFolders.length === 0) {
    console.info(
      `No subfolders found in ${sourceDirectoryAbs}. Nothing to add.`,
    );
  ...
  const config =
    (parsed.config as ts.server.ProjectInfoTelemetryEventData) ?? {};

  const compilerOptions = ensurePlainObject(
    config,
    'compilerOptions',
  ) as CompilerOptions;

  const paths = (ensurePlainObject(compilerOptions, 'paths') || {}) as Record<
    string,
    string[]
  >;
  ...
  const baseForPathsAbs = baseUrl
    ? path.resolve(tsconfigDirectory, baseUrl)
    : tsconfigDirectory;

  const added = [];

  // Keep existing order, append new keys in a stable order.
  for (const directoryName of sourceFolders.toSorted(
